          ...
          <tbody>
            {items.map((item, ind) => (
              <tr key={item._id ?? ind} className="tr-tbody">
                ...
                <td className="py-4">{item.returnDate ? new Date(item.returnDate).toLocaleDateString() : "—"}</td>
                <td className="py-4">{item.totalAmount ?? "—"}</td>
                <td className="py-4">{item.status ?? "—"}</td>
                <td className="py-4 flex justify-center gap-2">
                  <RowActions
                    label="return"
                    endpoint={`/returns/${item._id}`}
                    onEdit={() => {
